...
def main():
    df = getHV(6994)
    voltages = df.groupby(["channel"])["voltage"].mean()
    ...
    for i,volt in enumerate(voltages):
        _point1 = Point("Muon_Veto_Voltage").tag("channel", i).field("voltage", voltages[i])

        write_api.write(bucket="hvTest", record=_point1)
