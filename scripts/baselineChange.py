...
def main():
    ...
    baseline_arr = np.linspace(500,15000, 29)

    zz = np.zeros(len(baseline_arr))
    err = np.zeros(len(baseline_arr))

    for i,baseline in enumerate(baseline_arr):
        print(i)
        args = ['waveform[100:'+str(baseline)+']', 'bl_mean1', 'bl_std1', 'bl_slope1', 'bl_intercept1']
        dsp_config['processors']['bl_mean1, bl_std1, bl_slope1, bl_intercept1']['args'] = args
    ...
    df = pd.DataFrame({"FWHM": zz, "baseline": baseline_arr, "error": err})
    df.to_csv("BaseNoiseChange1725.csv")
    ...
    df = pd.DataFrame({"FWHM": zz, "rise": rise_arr, "flat": flat, "error": err})
    df.to_csv("RiseNoiseChange1725.csv")
    """
